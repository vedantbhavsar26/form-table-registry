...
const parseContent = (content: string): string => {
  return (
    content
      ...
      .replace(/\\/g, '\\\\')
      // Escape backticks (important for template literals)
      .replace(/`/g, '\\`')
      // Escape ${ so it doesn’t interpolate
      .replace(/\$\{/g, '\\${')
      // Escape double quotes (useful if embedding inside JSON or JS strings)
      .replace(/"/g, '\\"')
      // Normalize CRLF → LF
      .replace(/\r\n/g, '\n')
      // Escape newlines
...
export const Index: Record<string, any> ={`;
  ...
  for (const item of registry.items) {
    ...
    const sourcePath = path.join(process.cwd(), item.files[0].path);
